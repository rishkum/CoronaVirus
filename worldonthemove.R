...
p.tvl.uk <- df %>% filter(region == "UK") %>%   
  ...
  geom_smooth() + scale_y_continuous(labels = function(x) paste0(x-100, "%")) +
...
# Preety fy the graph
p.tvl.us <- ggpretay(p.tvl.uk, title = "US seems to be on the move too", 
                     subtitle = "Data from Apple's mobility data suggests that Americans have started to travel again.
...
# Preety fy the graph
p.tvl.wld <- ggpretay(p.tvl.uk, title = "World is getting back on the move", 
...
p.vst.wld1 <- p.vst.wld +
  ...
  geom_curve(curvature = -0.15, color = "dark gray",
             # Position , no quotes cause continous
             #0.03 is the size, curvature is the shape
             aes(x = as.Date("2020-01-30"), y = 50, xend = as.Date("2020-03-15"), yend = 160.00),
             arrow = arrow(length = unit(0.02, "npc"))) + 
  # text
  annotate("text", x = as.Date("2020-04-30"),
           y = 150,
